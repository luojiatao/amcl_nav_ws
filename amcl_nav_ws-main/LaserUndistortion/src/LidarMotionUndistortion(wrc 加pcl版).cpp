...
public:
    ...
    {
        tf_ = tf;
        std::cout<<"start subscribe scan"<<std::endl;
        ...
        scan_pub_raw = nh_.advertise<sensor_msgs::LaserScan>("/scan_raw", 1000);
    }
    //析构函数，释放tf_
    ...
    void ScanCallBack(const champion_nav_msgs::ChampionNavLaserScanPtr& scan_msg)
    {
       ...
       champion_nav_msgs::ChampionNavLaserScan laserScanMsg = *scan_msg;
        ...
        for(int i = 0; i < beamNum;i++)
        {
            ...
            double lidar_angle = laserScanMsg.angles[i];//单位弧度
        ...
        scan_raw_pub(ranges,startTime,ros::Duration(laserScanMsg.time_increment * beamNum).toSec());
        ...
        std::cout<<"Publish Scan_cal!"<<std::endl;
    ...
    //矫正之qian的scan数据的发布函数，这里默认发布所有360度的所有数据,正着安装,所填写的参数是rplidar A1
    void scan_raw_pub(const std::vector<double> &ranges,ros::Time pubTime,double scan_time)
    {
    ...
    void scan_cal_pub(const std::vector<double> &ranges,ros::Time pubTime,double scan_time)
    {
        ...
        visual_cloud_scan(ranges_temp,angles_temp,0,0,0,pubTime);
        //#endif
        //===========================================================================
    }
...
public:
    ...
    ros::Publisher  scan_pub_raw;

    //针对各自的情况需要更改的名字，自行更改
    const std::string scan_frame_name_="base_laser";
    const std::string odom_name_="odom";
    const std::string scan_sub_name_="champion_scan";
...
int main(int argc,char ** argv)
{
    ...
    std::cout<<"strat listern tf"<<std::endl;
