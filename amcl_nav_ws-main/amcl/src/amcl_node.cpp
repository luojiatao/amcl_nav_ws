...
class AmclNode
{
  ...
  private:
    ...
    void laserReceived(const champion_nav_msgs::ChampionNavLaserScanPtr& laser_scan);
    ...
    //message_filters::Subscriber<champion_nav_msgs::ChampionNavLaserScan>* laser_scan_sub_;
    //tf2_ros::MessageFilter<champion_nav_msgs::ChampionNavLaserScan>* laser_scan_filter_;
    ...
    ros::Subscriber scan_sub_; //声明scan的订阅者
        ...
        first_reconfigure_call_(true)
{
  ...
  scan_sub_ = nh_.subscribe(scan_topic_, 100, &AmclNode::laserReceived, this);
/*
  laser_scan_sub_ = new message_filters::Subscriber<champion_nav_msgs::ChampionNavLaserScan>(nh_, scan_topic_, 100);
  laser_scan_filter_ = 
          new tf2_ros::MessageFilter<champion_nav_msgs::ChampionNavLaserScan>(*laser_scan_sub_,
                                                             *tf_,
                                                             odom_frame_id_,
                                                             100,
                                                             nh_);
...
void AmclNode::runFromBag(const std::string &in_bag_fn, bool trigger_global_localization)
{
  ...
  ros::Publisher laser_pub = nh_.advertise<champion_nav_msgs::ChampionNavLaserScan>(scan_topic_name, 100);
...
void AmclNode::savePoseToServer()
{
  ...
  //std::cout<<"odom_pose_tf2 : "<<latest_odom_pose_.pose<<std::endl;
...
void
AmclNode::laserReceived(const champion_nav_msgs::ChampionNavLaserScanPtr& laser_scan)
{
  ...
  if(frame_to_laser_.find(laser_scan_frame_id) == frame_to_laser_.end())
  {
    ...
    //std::cout<<"start base->base_laser "<<std::endl;
  ...
  pf_vector_t pose;
  //std::cout<<"start getOdom_pose : "<<std::endl;
  ...
  if(lasers_update_[laser_index])
  {
    ...
    q.setRPY(0.0, 0.0,laser_scan->angles[1]);
    ...
    for(int i=0;i<ldata.range_count;i++)
    {
      ...
      // Compute bearing
      ldata.ranges[i][1] = laser_scan->angles[i];
    }
    //std::cout<<"start UpdateSensor : "<<std::endl;
  ...
  {
      std::cout<<"resampled || force_publication "<<std::endl;
        ...
        hyp_count < pf_->sets[pf_->current_set].cluster_count; hyp_count++)
    {
     ...
     std::cout<<"hyps[hyp_count].weight = weight"<<std::endl;
    ...
    //std::cout<<"Max weight is  : "<<max_weight<<std::endl;
    if(max_weight > 0.0)
    {
      std::cout<<"Max weight is  : "<<max_weight<<std::endl;
